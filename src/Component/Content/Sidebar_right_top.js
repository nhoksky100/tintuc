...
class Sidebar_right_bottom extends Component {
    ...
    componentWillMount() {
        var data = datafirebase.database().ref('TinTheThao');
       data.on('value', (notes) => {
            var datapush = [];
    ...
    isShow = () => {
        ...
        return (
            <div className="col shadow mt-5 ml bg ">
                <div className="filter-control">
                    <h3 className="text-muted float-left">Tin Thể Thao</h3>
                </div><br />
                    ...
                    return (
                            ...
                            <ul className="product-item row" key={key} >
                                <li className="list card-text col-4">
                                    <Link to={"/chi-tiet/TinTheThao/" + stringtoslug(value.title) + "." + key + ".html"} className="nav-link" >
